...
;if (Drupal.jsEnabled) {
  $(document).ready(function(){
	//$('#newsletter').example('e-mail');
	//if ($messages){ $.prompt($messages);
	var porukice = $("div#messages").html();
	if (porukice) { $.prompt(porukice)} 
	//}
	...
	$('#proizvod-slideshow').cycle({ 
	    fx:     'fade', 
	    speed:   1500, 
	    timeout: 3500
		});
	$('#block-views-proizvodi_naslovna-block_2 div.content').cycle({ 
	    fx:     'fade', 
	    speed:   1500, 
	    timeout: 3500
		});
    ...
    $('#pernodricard-pitanje').submit(function() { 
	...
	$("#pernodricard-pitanje").ajaxSuccess(function(evt, request, settings){	
	});
	...
	var options2 = { 
		...
		//success: function() { 
            //$('#ajax-response').fadeIn('slow'); 
			//} 
		}; 
	$('#simplenews-block-form-1').ajaxForm(options2);
	var options3 = { 
		target:     '#ajax-response',
        success:       showResponse
		};
	$('#simplenews-confirm-removal-form').ajaxForm(options2);
...
function validate(formData, jqForm, options) { 
	...
	*/
	//nisu uneti podaci	
    if (!form.ime.value) { 
		msgErrorText += "Unesite polje: ime i prezime <br />";
    } 
	if (!form.body.value) { 
	 	msgErrorText += "Unesite polje: tekst poruke <br />";
    } 	
	//unet neispravan email
	if (!form.email.value) { 
	 	msgErrorText += "Unesite polje: e-mail <br />";
    }
	else {
	  var filter = /^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$/i;
      if (!filter.test(form.email.value)) {
	    msgErrorText += "Uneta e-mail adresa nije validna <br />";
	  }
    }
	//print error msg
	if(msgErrorText.length!=0){ 
		alert_lightbox(msgErrorText);
		return false;
	}
	/*
	if validation is OK
...
function showResponse(responseText, statusText)  {
	if(responseText == "Uspešno ste poslali poruku. Hvala.") {
...
function validate2(formData, jqForm, options) { 
	...
	if (!form2.mail.value) { 
	  ...
	  var filter = /^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$/i;
      if (!filter.test(form2.mail.value)) {
	    msgErrorText += "Uneta e-mail adresa nije validna <br />";
	  }
    }
	//print error msg
